...
//백링크 검색부분
router.get('/backsearch_api', (req, res) => {
    const db = req.app.db;
        // get the top results based on sort order
        common.dbQuery(db.kb, { kb_published: 'true' }, null, null, (err, featured_results) => {
            res.json(featured_results); //res.send('<script>console.log('+featured_results+')</script>');
    });
});
...
router.post('/insert_kb', common.restrict, (req, res) => {
    ...
    db.kb.count({ kb_permalink: req.body.frm_kb_permalink }, (err, kb) => {
        if (kb > 0 && req.body.frm_kb_permalink !== '') {
            ...
            common.validate_title(db, doc, (err, result) => {
                // keep the current stuff
                req.session.kb_title = req.body.frm_kb_title;
                req.session.kb_body = req.body.frm_kb_body;
                req.session.kb_keywords = req.body.frm_kb_keywords;
                ...
                if (!err) {
                    ...
                    db.kb.insert(doc, (err, newDoc) => {
                        if (err) {
                            ...
                            req.session.message = req.i18n.__('새로운 article 생성완료');
                            req.session.message_type = 'success';
                            res.redirect(req.app_context + '/kfpa/' + newId);
                            //res.redirect('/'); 
                        }
                    });
                } else {
                    req.session.message = req.i18n.__('중복된 Title입니다.');
                    req.session.message_type = 'danger';
                    res.redirect(req.app_context + '/insert');
                }
            });
...
let link_paste = ''
// insert new KB form action
router.post('/insert_kb2', common.restrict, (req, res) => {
    const db = req.app.db;
    const lunr_index = req.app.index;

    const doc = {
        kb_permalink: req.body.frm_kb_permalink2,
        kb_title: req.body.frm_kb_title2,
        kb_body: req.body.frm_kb_body2,
        kb_published: req.body.frm_kb_published2,
        kb_keywords: req.body.frm_kb_title2 + ',해시태그추가예정',
        kb_published_date: new Date(),
        kb_last_updated: new Date(),
        kb_last_update_user: req.session.users_name + ' - ' + req.session.user,
        kb_author: req.session.users_name,
    ...
    db.kb.count({ kb_permalink: req.body.frm_kb_permalink2 }, (err, kb) => {
        if(kb > 0 && req.body.frm_kb_permalink2 !== ''){
            // permalink exits
            req.session.message = req.i18n.__('Permalink already exists. Pick a new one.');
            req.session.message_type = 'danger';

            // keep the current stuff
            req.session.kb_title = req.body.frm_kb_title2;
            req.session.kb_body = req.body.frm_kb_body2;
            req.session.kb_keywords = req.body.frm_kb_title2;
            req.session.kb_permalink = req.body.frm_kb_permalink2;
            ...
            db.kb.insert(doc, (err, newDoc) => {
                if(err){
                    ...
                    // setup keywords스트을위한추가페이지1
                    let keywords = '';
                    if(req.body.frm_kb_title2 + ',해시태그추가예정' !== undefined){
                        keywords = req.body.frm_kb_title2 + ',해시태그추가예정'.toString().replace(/,/g, ' ');
                        console.log(keywords,'ㅣ이프문')
                    }else{
                    keywords = req.body.frm_kb_title2 + ',해시태그추가예정'.toString();
                    console.log(keywords,'엘즈문')
                    }
                    // get the new ID
                    let newId = newDoc._id;
                    if(config.settings.database.type !== 'embedded'){
                    ...
                    // if index body is switched on
                    if(config.settings.index_article_body === true){
                        lunr_doc['kb_body'] = req.body.frm_kb_body2;
                    }
                    // add to lunr index
                    lunr_index.add(lunr_doc);
                    //req.session.message = req.i18n.__('새로운 article 생성완료');
...
//         kb_body: req.body.frm_kb_body2,
//         kb_published: req.body.frm_kb_published2,
//         kb_keywords: req.body.frm_kb_title2,
//         kb_published_date: new Date(),
//         kb_last_updated: new Date(),
//         kb_last_update_user: req.session.users_name + ' - ' + req.session.user,
//         kb_author: req.session.users_name,
...
//                     // setup keywords
//                     let keywords = '';
//                     if(req.body.frm_kb_title2 !== undefined){
//                         keywords = req.body.frm_kb_title2.toString().replace(/,/g, ' ');
//                     }else{
//                     keywords = req.body.frm_kb_title2.toString();
//                 }
//                     // get the new ID
//                     let newId = newDoc._id;
//                     if(config.settings.database.type !== 'embedded'){
...
//                     //res.send("<script>document.location.href='/edit/63dcaa9e40af886740f64c55'</script>")
//                     // redirect to new doc
...
router.get('/link_check', (req, res) => {
    playAlert = setInterval(() => {
        if(link_paste !== '') {
          
            clearInterval(playAlert);
            res.json(200, link_paste)
        } else {
            console.log('에러!')
        }
    }, 1000)
...
router.post('/save_kb', common.restrict, (req, res) => {
    ...
    db.kb.count({ kb_permalink: req.body.frm_kb_permalink, $not: { _id: common.getId(req.body.frm_kb_id) }, kb_versioned_doc: { $ne: true } }, (err, kb) => {
        if(kb > 0 && req.body.frm_kb_permalink !== ''){
            ...
            req.session.kb_seo_title = req.body.frm_kb_title;
            ...
            db.kb.findOne({ _id: common.getId(req.body.frm_kb_id) }, (err, article) => {
                ...
                db.kb.update({ _id: common.getId(req.body.frm_kb_id) }, {
                    $set: {
                        ...
                        kb_seo_title: req.body.frm_kb_title,
                    ...
                    if(err){
                        ...
                        if(article_versioning === true){
                            ...
                            db.kb.insert(version_doc, (err, version_doc) => {
                                req.session.message = req.i18n.__('Successfully saved1');
                                req.session.message_type = 'success';
                                res.redirect(req.app_context + '/edit/' + req.body.frm_kb_id);
                            });
                        }else{
                            req.session.message = req.i18n.__('변경내용이 저장되었습니다.');
                            req.session.message_type = 'success';
                            //res.send(<script>alert('임시저장완료')</script>)
                            //res.redirect(req.app_context + '/edit/' + req.body.frm_kb_id);
                            res.redirect(req.app_context + '/kfpa/' + req.body.frm_kb_id);
...
// Update an existing KB article form action - 최종저장, 중간저장 api와 추후에 합치기
router.post('/save_kb2', common.restrict, (req, res) => {
    const db = req.app.db;
    const lunr_index = req.app.index;
    const kb_featured = req.body.frm_kb_featured === 'on' ? 'true' : 'false';
...
//save_kb3 정리하기  (savs한개로 만들기 가능? 단, 인자들이다다름 효율적인함수찾기)
// Update an existing KB article form action
// router.post('/save_kb3', common.restrict, (req, res) => {
//     const db = req.app.db;
//     const lunr_index = req.app.index;
//     const kb_featured = req.body.frm_kb_featured === 'on' ? 'true' : 'false';
...
// custom
router.get('/node', common.restrict, (req, res) => {
    const db = req.app.db;
    common.dbQuery(db.kb, { kb_versioned_doc: { $ne: true } }, { kb_published_date: -1 }, 10, (err, articles) => {
        res.render('node', {
            title: 'node',
            ...
            if(bcrypt.compareSync(req.body.password, user.user_password) === true){
                ...
                if(req.body.frm_referring_url === undefined || req.body.frm_referring_url === ''){
                    ...
                    if(url_parts.pathname !== '/setup' && url_parts.pathname !== req.app_context + '/login'){
                        ...
                        //location.href = '/'; //로그인 성공시 기본페이지 로드
                    }else{
                        res.redirect(req.app_context + '/');
                    }
                }
            }else{
                console.log('이메일 있음, 비밀번호틀림')
